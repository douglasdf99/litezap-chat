...
const ShareChatbot = ({ isSessionMemory }) => {
  ...
  const [isPublicChatflow, setChatflowIsPublic] = useState(
    chatflow.isPublic ?? false
  );
  const [generateNewSession, setGenerateNewSession] = useState(
    chatbotConfig?.generateNewSession ?? false
  );
  ...
  const [backgroundColor, setBackgroundColor] = useState(
    chatbotConfig?.backgroundColor ?? defaultConfig.backgroundColor
  );
  const [fontSize, setFontSize] = useState(
    chatbotConfig?.fontSize ?? defaultConfig.fontSize
  );
  const [poweredByTextColor, setPoweredByTextColor] = useState(
    chatbotConfig?.poweredByTextColor ?? defaultConfig.poweredByTextColor
  );

  const [botMessageBackgroundColor, setBotMessageBackgroundColor] = useState(
    chatbotConfig?.botMessage?.backgroundColor ??
      defaultConfig.botMessage.backgroundColor
  );
  ...
  const [botMessageAvatarSrc, setBotMessageAvatarSrc] = useState(
    chatbotConfig?.botMessage?.avatarSrc ?? ""
  );
  const [botMessageShowAvatar, setBotMessageShowAvatar] = useState(
    chatbotConfig?.botMessage?.showAvatar ?? false
  );

  const [userMessageBackgroundColor, setUserMessageBackgroundColor] = useState(
    chatbotConfig?.userMessage?.backgroundColor ??
      defaultConfig.userMessage.backgroundColor
  );
  ...
  const [userMessageAvatarSrc, setUserMessageAvatarSrc] = useState(
    chatbotConfig?.userMessage?.avatarSrc ?? ""
  );
  const [userMessageShowAvatar, setUserMessageShowAvatar] = useState(
    chatbotConfig?.userMessage?.showAvatar ?? false
  );

  const [textInputBackgroundColor, setTextInputBackgroundColor] = useState(
    chatbotConfig?.textInput?.backgroundColor ??
      defaultConfig.textInput.backgroundColor
  );
  const [textInputTextColor, setTextInputTextColor] = useState(
    chatbotConfig?.textInput?.textColor ?? defaultConfig.textInput.textColor
  );
  const [textInputPlaceholder, setTextInputPlaceholder] = useState(
    chatbotConfig?.textInput?.placeholder ?? ""
  );
  const [textInputSendButtonColor, setTextInputSendButtonColor] = useState(
    chatbotConfig?.textInput?.sendButtonColor ??
      defaultConfig.textInput.sendButtonColor
  );
  ...
  const formatObj = () => {
    ...
    if (userMessageShowAvatar)
      obj.userMessage.showAvatar = userMessageShowAvatar;

    if (textInputBackgroundColor)
    ...
    if (textInputSendButtonColor)
      obj.textInput.sendButtonColor = textInputSendButtonColor;

    if (isSessionMemory)
      obj.overrideConfig.generateNewSession = generateNewSession;

    if (chatbotConfig?.starterPrompts)
      obj.starterPrompts = chatbotConfig.starterPrompts;
  ...
  return (
    <>
      <Stack direction="row">
        ...
        <div style={{ display: "flex", alignItems: "center" }}>
          ...
          <TooltipWithParser
            style={{ marginLeft: 10 }}
            title={
              "Making public will allow anyone to access the chatbot without username & password"
            }
          />
        </div>
      </Stack>
      ...
      {textField(
        ...
        "Welcome Message",
        "string",
        "Hello! This is custom welcome message"
      )}
      ...
      {isSessionMemory && (
        <>
          ...
          {booleanField(
            generateNewSession,
            "generateNewSession",
            "Start new session when chatbot link is opened or refreshed"
          )}
        </>
      )}
      ...
      >
        <SketchPicker
          ...
          onChange={(color) => onColorSelected(color.hex)}
        />
      </Popover>
      <Popover
